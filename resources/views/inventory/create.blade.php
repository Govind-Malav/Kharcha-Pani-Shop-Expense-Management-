...
@section('content')
<div class="space-y-6 max-w-4xl mx-auto" x-data="{ cost: 0, sell: 0 }">
    ...
    <div class="flex items-center space-x-2 text-xs font-semibold text-slate-400">
        ...
        <span><i class="fa-solid fa-chevron-right text-3xs"></i></span>
        <span class="text-slate-600">Add Product</span>
    </div>

    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Add Product to Inventory</h1>
        <p class="text-sm text-slate-500 mt-1">Register retail products, set purchase vs retail pricing, and define low stock threshold notifications.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <form method="POST" action="{{ route('inventory.store') }}" enctype="multipart/form-data" class="p-6 md:p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="e.g. Parle-G Biscuit 100g, Amul Gold Milk 1L" 
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="cost_price" id="cost_price" step="0.01" min="0" required placeholder="0.00" 
                               x-model.number="cost"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="selling_price" id="selling_price" step="0.01" min="0" required placeholder="0.00" 
                               x-model.number="sell"
                               class="w-full pl-8 pr-4 py-2.5 text-sm rounded-xl border-slate-200 focus:border-teal-500 focus:ring focus:ring-teal-200/50 transition-colors">
                    </div>
                </div>

                {{-- Dynamic Profit Calculations Preview --}}
                ...
                {{-- Initial Quantity --}}
                <div>
                    <label for="quantity" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Initial Quantity *</label>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="barcode" id="barcode" value="{{ old('barcode') }}" placeholder="Scan or enter code" 
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="supplier_id" id="supplier_id" 
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }} ({{ $supplier->phone ?? 'No Phone' }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div x-data="{ imgName: '' }">
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="image" class="flex flex-col items-center justify-center w-full h-24 border-2 border-slate-200 border-dashed rounded-2xl cursor-pointer bg-slate-50/50 hover:bg-slate-50 transition-colors">
                            <div class="flex flex-col items-center justify-center pt-2 pb-2 text-center px-4">
                                ...
                                <span class="text-3xs font-bold text-slate-500 truncate max-w-[180px]" x-text="imgName ? imgName : 'Upload photo'"></span >
                            </div>
                ...
                <div class="md:col-span-3">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Enter optional notes about catalog specs, location in warehouse, rack row..." 
