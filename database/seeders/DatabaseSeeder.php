...
class DatabaseSeeder extends Seeder
{
    ...
    {
        // 1. Seed Users (using firstOrCreate to prevent unique constraint crashes)
        ...
        foreach ($categories as $cat) {
            $categoryModels[$cat['name']] = ExpenseCategory::firstOrCreate(
                ['name' => $cat['name']],
                ['description' => $cat['description'], 'is_custom' => $cat['is_custom']]
            );
        }
        ...
        $products = [
            [
                ...
                'supplier_id' => $supplierModels[0]->id ?? null,
            ...
            [
                ...
                'supplier_id' => $supplierModels[1]->id ?? null,
            ...
            [
                ...
                'supplier_id' => $supplierModels[2]->id ?? null,
        ...
        // 7. Seed Historical Expenses (Last 30 Days) - only if table is empty
        if (Expense::count() === 0) {
            $now = Carbon::now();
            Expense::create([
        ...
        // 8. Seed Sales & SaleItems (Last 10 Days) - only if table is empty
        if (Sale::count() === 0) {
            $now = Carbon::now();
            $sale1 = Sale::create([
        ...
        // 9. Seed Credits - only if table is empty
        if (Credit::count() === 0) {
            $now = Carbon::now();
            // Customer Credit
